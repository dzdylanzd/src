		...
		int aFreeRadius /*= 1*/)
{
	...
	const std::vector<Model::WallPtr> &walls =
			Model::RobotWorld::getRobotWorld().getWalls();
 ...
 *
 */
Path AStar::search(const Point &aStartPoint, const Point &aGoalPoint,
...
Path AStar::search(Vertex aStart, const Vertex &aGoal, const Size &aRobotSize)
{
	...
	int radius = static_cast<int>(std::ceil(
			std::sqrt(
					(aRobotSize.x / 2.0) * (aRobotSize.x / 2.0)
							+ (aRobotSize.y / 2.0) * (aRobotSize.y / 2.0))));
